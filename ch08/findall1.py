...
# '*', '+'의 차이
"""
p = re.compile('ca*t')       # *: 앞 문자가 0번 이상 반복(없어도 찾음)
m1 = re.findall(p, 'caat')
print(m1)   # ['caat']
m2 = re.findall(p, 'ct')
print(m2)   # ['ct']

p2 = re.compile('ca+t')      # +: 앞 문자가 1번 이상 반복(없으면 못찾음)
...
print(m1)   # ['hr'] <>안의 문자 출력(태그 미포함), 어떤 문자든 0번 이상 나오면 찾음

pat2 = re.compile("(<.*>)")
m2 = re.findall(pat2, str)
print(m2)   # ['<hr>'] <>포함 문자 출력(태그 포함)
"""
